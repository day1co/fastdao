...
import { TransactingOps, Transacting } from './transacting-operations';
import { Sort, Relation } from '..';
    ...
    Transacting<ID, ROW>
{
  ...
  private readonly transactingOps: TransactingOps<ID, ROW>;
  constructor(opts: MysqlCrudOperationsOpts<ID, ROW>) {
    ...
    this.transactingOps = new TransactingOps(opts);
  }
  //---------------------------------------------------------
  // CustomOperation
  // 공용 함수를 사용하지 않는 기능이 만들고 싶다면 이곳에
  // async findBySiteAndXxx(filter?: CrudFilter<ID, ROW>, sorts?: Array<Sort>, relations?: Array<Relation>): Promise<Array<ROW>> {
  // }
  ...
  transacting(tx: Knex.Transaction): TransactingOps<ID, ROW> {
    return this.transactingOps.transacting(tx);
  }
}
